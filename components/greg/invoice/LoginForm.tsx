...
export default function LoginForm({
  ...
  async function submit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const res = await fetch('/api/greg/admin/session', {
        method: 'POST',
    ...
    <div className="mx-auto w-full max-w-sm rounded-2xl border border-border-subtle bg-bg-secondary p-7">
      ...
      <form onSubmit={submit} className="mt-6 flex flex-col gap-3">
        ...
        <button
          ...
          className="mt-1 inline-flex items-center justify-center gap-2 rounded-lg bg-cta-bg px-5 py-3 font-semibold text-cta-fg transition-all hover:brightness-95 disabled:cursor-not-allowed disabled:opacity-50"
        >
          {loading ? (
