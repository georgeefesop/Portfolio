...
interface HeroTextProps {
    ...
    step?: StepId;
    onOpenDemo?: () => void; // New: callback to open modal in simple mode
}

export default function HeroText({ scrollProgress, step, onOpenDemo }: HeroTextProps) {
    const y = useTransform(scrollProgress, [0, 1], [0, -20]);
    const opacity = useTransform(scrollProgress, [0, 0.5], [1, 0]);

    // Simple mode = no carousel (onOpenDemo provided)
    ...
    return (
        ...
        >
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end w-full gap-8 md:gap-0">
                    ...
                    {isSimpleMode && (
                        ...
                        >
                            <p className="text-zinc-300 text-base md:text-lg leading-relaxed max-w-xl">
                                I design solutions to messy problems, then build prototypes you can test immediately.
                            </p>
                            <button
                                onClick={onOpenDemo}
                                className="pointer-events-auto group inline-flex items-center gap-2 px-6 py-3 bg-accent-primary hover:bg-accent-primary/90 text-black font-bold text-sm md:text-base rounded-lg transition-all duration-300 shadow-lg hover:shadow-xl hover:scale-105"
                                aria-label="Open live POS demo"
                            >
                ...
                {/* Right Side: Details + CTA (Hidden on mobile, only in carousel mode) */}
                {!isSimpleMode && (
                    <motion.div
                        variants={itemVariants}
